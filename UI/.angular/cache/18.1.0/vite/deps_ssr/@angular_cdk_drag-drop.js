...
var DragRef = class {
  ...
  constructor(element, _config, _document, _ngZone, _viewportRuler, _dragDropRegistry) {
    ...
    this._moveEvents = new import_rxjs.Subject();
    this._pointerMoveSubscription = import_rxjs.Subscription.EMPTY;
    this._pointerUpSubscription = import_rxjs.Subscription.EMPTY;
    this._scrollSubscription = import_rxjs.Subscription.EMPTY;
    ...
    this.beforeStarted = new import_rxjs.Subject();
    this.started = new import_rxjs.Subject();
    this.released = new import_rxjs.Subject();
    this.ended = new import_rxjs.Subject();
    this.entered = new import_rxjs.Subject();
    this.exited = new import_rxjs.Subject();
    this.dropped = new import_rxjs.Subject();
...
var DropListRef = class {
  constructor(element, _dragDropRegistry, _document, _ngZone, _viewportRuler) {
    ...
    this.exited = new import_rxjs.Subject();
    this.dropped = new import_rxjs.Subject();
    this.sorted = new import_rxjs.Subject();
    this.receivingStarted = new import_rxjs.Subject();
    this.receivingStopped = new import_rxjs.Subject();
    this._isDragging = false;
    this._draggables = [];
    this._siblings = [];
    this._activeSiblings = /* @__PURE__ */ new Set();
    this._viewportScrollSubscription = import_rxjs.Subscription.EMPTY;
    this._verticalScrollDirection = AutoScrollVerticalDirection.NONE;
    this._horizontalScrollDirection = AutoScrollHorizontalDirection.NONE;
    this._stopScrollTimers = new import_rxjs.Subject();
    this._cachedShadowRoot = null;
    this._scrollableElements = [];
    this._direction = "ltr";
    this._startScrollInterval = () => {
      this._stopScrolling();
      (0, import_rxjs.interval)(0, import_rxjs.animationFrameScheduler).pipe((0, import_operators.takeUntil)(this._stopScrollTimers)).subscribe(() => {
...
var _DragDropRegistry = class _DragDropRegistry {
  constructor(_ngZone, _document) {
    ...
    this.scroll = new import_rxjs.Subject();
  ...
  scrolled(shadowRoot) {
    ...
    if (shadowRoot && shadowRoot !== this._document) {
      streams.push(new import_rxjs.Observable((observer) => {
        return this._ngZone.runOutsideAngular(() => {
    ...
    return (0, import_rxjs.merge)(...streams);
  }
  ngOnDestroy() {
...
var _CdkDrag = class _CdkDrag {
  ...
  constructor(element, dropContainer, _document, _ngZone, _viewContainerRef, config, _dir, dragDrop, _changeDetectorRef, _selfHandle, _parentDrag) {
    ...
    this._handles = new import_rxjs.BehaviorSubject([]);
    ...
    this.moved = new import_rxjs.Observable((observer) => {
      const subscription = this._dragRef.moved.pipe((0, import_operators.map)((movedEvent) => ({
        source: this,
  ...
  _syncInputs(ref) {
    ...
    ref.beforeStarted.pipe((0, import_operators.take)(1)).subscribe(() => {
      if (this._parentDrag) {
  ...
  _setupHandlesListener() {
    this._handles.pipe(
      ...
      (0, import_operators.tap)((handles) => {
      ...
      (0, import_operators.switchMap)((handles) => {
        return (0, import_rxjs.merge)(...handles.map((item) => item._stateChanges.pipe((0, import_operators.startWith)(item))));
      }),
...
var _CdkDropList = class _CdkDropList {
  ...
  _setupInputSyncSubscription(ref) {
    if (this._dir) {
      this._dir.change.pipe((0, import_operators.startWith)(this._dir.value), (0, import_operators.takeUntil)(this._destroyed)).subscribe((value) => ref.withDirection(value));
    }
    ref.beforeStarted.subscribe(() => {
  ...
  _handleEvents(ref) {
    ...
    (0, import_rxjs.merge)(ref.receivingStarted, ref.receivingStopped).subscribe(() => this._changeDetectorRef.markForCheck());
  }
  /** Assigns the default input values based on a provided config object. */
  _assignDefaults(config) {
    const {
