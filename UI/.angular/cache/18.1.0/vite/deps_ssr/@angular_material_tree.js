...
var MatTreeFlatDataSource = class extends DataSource {
  ...
  constructor(_treeControl, _treeFlattener, initialData) {
    ...
    this._flattenedData = new import_rxjs.BehaviorSubject([]);
    this._expandedData = new import_rxjs.BehaviorSubject([]);
    this._data = new import_rxjs.BehaviorSubject([]);
    if (initialData) {
      this.data = initialData;
    }
  }
  connect(collectionViewer) {
    return (0, import_rxjs.merge)(collectionViewer.viewChange, this._treeControl.expansionModel.changed, this._flattenedData).pipe((0, import_operators.map)(() => {
...
var MatTreeNestedDataSource = class extends DataSource {
  ...
  connect(collectionViewer) {
    return (0, import_rxjs.merge)(...[collectionViewer.viewChange, this._data]).pipe((0, import_operators.map)(() => this.data));
  }
  disconnect() {
  }
};
